...
PORT = 8080
HOST = "127.0.0.1"
APP_VERSION = "3.1"          # phase-3 build (ported PWA refinements)
...
let _dragDepth=0;
window.addEventListener("dragenter", e=>{ e.preventDefault(); _dragDepth++; document.body.classList.add("dragging"); });
...
function lazyThumbs(){
  ...
  $("modal").querySelectorAll("img[data-url]").forEach(i=>modalObserver.observe(i));
}
...
function openCopyPages(){
  if(pages<=0) return;
  copySel=new Set();
  renderCopyModal(); $("overlay").classList.add("show");
}
  ...
  const thumbs=[...Array(pages).keys()].map(i=>{
    ...
    <div class="thumb ${copySel.has(i)?"picked":""}" onclick="copyToggle(${i})"
      ...
      <img data-url="/api/page?n=${i}&w=120" data-key="${curEpoch}:${i}:120"
        width="120" alt="page ${i+1}" style="background:#fff">
    </div>`;}).join("");
  $("modal").innerHTML=`
